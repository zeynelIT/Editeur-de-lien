...
cat MyReadelfCommand.output | while read line || [ -n "$line" ]; do
    ...
    if [ "$line" == "There is no data to dump." ]; then
        ...
        if [ ! -z "$otherValue" ]; then #On vérifie si la variable n'est pas vide, e.g: Si grep a trouvé la ligne
            # Trouvé, readelf -x est d'accord avec notre programme. Pas de tests supplémentaires nécessaires
            echo -e "No data : \033[48;5;2mOK TEST\033[0;0m" #Pass
            echo
            echo -e "Test $(basename "$1") \033[48;5;2mpassed\033[0;0m!"
            rm -f MyReadelfCommand.output readelfCommand.output
    ...
    [[:xdigit:]]*)
        ...
        if [ "$otherValue" = "$value" ]; then # Les résultats correspondent on passe à la ligne suivante
            :
...
echo -e "Content : \033[48;5;2mOK TEST\033[0;0m" #Pass
echo
echo -e "Test $(basename "$1") \033[48;5;2mpassed\033[0;0m!"
rm -f MyReadelfCommand.output readelfCommand.output
exit 0
