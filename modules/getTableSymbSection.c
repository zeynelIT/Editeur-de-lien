...
void symTabName(FILE* file, Elf32_Sym* symtab, char verbose){
	fread(&symtab->st_name, 4, 1, file);
	if (verbose)
	{
		printOctet(&symtab->st_name, 4, 1);
		printf("\t");
	}
}
...
void getTabSymb(FILE* file, Elf32_Sym* symtab, char verbose){
	symTabName(file, symtab, verbose);
