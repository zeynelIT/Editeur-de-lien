...
errorReadelf=$?
./readelf_all --file $1 --type S &>MyReadelfCommand.output
errorMyReadelf=$?
...
cat MyReadelfCommand.output | while read line || [ -n "$line" ]; do
    ...
    \[[[:space:]*[0-9]*\])
        ...
        if [[ "$key" =~ $regexZero ]]; then #Match tous les [0], [ 0], [  0], ...
            # ==Alternate source==
            ...
            if [[ $otherFlags =~ $regexNumber ]]; then #Pas de flags puisque flag vide
            ...
            if [[ $otherFlags =~ $regexNumber ]]; then
                alternateSourceFlag=1
                otherLink=$(echo $otherValue | cut -d' ' -f7)
                otherInfo=$(echo $otherValue | cut -d' ' -f8)
                otherAlign=$(echo $otherValue | cut -d' ' -f9)
        ...
        if [ $alternateSourceName -eq 1 ]; then
        ...
        if [ "$execAddr" == "$otherExecAddr" ]; then
            echo -e "Execution Adress : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
        ...
        if [ $ans -eq 1 ]; then                             #Si les deux nombres sont égaux
            echo -e "Offset : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Offset : "
        ...
        if [ $ans -eq 1 ]; then                                   #Si les deux nombres sont égaux
            echo -e "Section Size : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Section Size : "
        ...
        if [ "$link" == "$otherLink" ]; then
            echo -e "Link To : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Link To : "
        ...
        if [ "$info" == "$otherInfo" ]; then
            echo -e "Info : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Info : "
        ...
        if [ "$align" == "$otherAlign" ]; then
            echo -e "Align : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Align : "
        ...
        if [ $ans -eq 1 ]; then                                 #Si les deux nombres sont égaux
            echo -e "Entry Size : \033[48;5;2mOK TEST\033[0;0m" #Pass
        else
            printf "Entry Size : "
